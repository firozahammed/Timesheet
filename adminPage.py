...
if radio_selection == 'Print Reports':
    ...
    if select_box_choice == 'Summary':
        ...
        df['Total Time']=pd.to_datetime(df['Total Time'],format='%H:%M:%S',errors = 'coerce').dt.time
        ...
        with pd.ExcelWriter(towrite, engine='xlsxwriter') as writer:
            ...
            writer.sheets['Sheet1'].write('A1', 'Date:' +(date_from).strftime("%m/%d/%Y") + " - " +(date_to).strftime("%m/%d/%Y"))
            #Autofit excel column header
            for column in df:
                column_length = max(df[column].astype(str).map(len).max(), len(column))
                col_idx = df.columns.get_loc(column)
        ...
        if ID != "":
            download_button = st.download_button(label="Download Report", data=towrite,
                                             file_name="Summary_"+str(ID)+"_"+str(date_from)+"-"+str(date_to)+".xlsx", mime="application/vnd.ms-excel")
        else:
            download_button = st.download_button(label="Download Report", data=towrite,
                                                 file_name="Summary_"+str(date_from)+"-"+str(date_to)+".xlsx", mime="application/vnd.ms-excel")
